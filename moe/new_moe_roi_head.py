...
class NewMoERoIHead(MetaRCNNRoIHead):
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        for img_id in range(batch_size):
            ...
            # 使用support_gt_labels中的随机一个类作为支持类
            random_index = np.random.choice(
            ...
            for i in range(support_feat.size(0)):
                if support_gt_labels[i] == random_query_label:
                    bbox_results = self._bbox_forward(
                        ...
                        support_feat_inv[i].sigmoid().unsqueeze(0)) # 使用VAE解码后的support_feat_inv作为支持特征
        ...
        if self.bbox_head.with_meta_cls_loss:
            ...
            # 使用VAE编码后的support_feat_rec作为支持特征
