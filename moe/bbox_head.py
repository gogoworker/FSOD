...
class BBoxHead(BaseModule):
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        Args:
            ...
            所有正样本框，形状为 (num_pos, 4)，这里的 4 代表边界框的坐标 [tl_x, tl_y, br_x, br_y]            
        ...
        #初始化为 self.num_classes，这个值代表背景类别
        ...
        if num_pos > 0:
            labels[:num_pos] = pos_gt_labels # 将正样本的标签设置为真实标签
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight # 正样本权重
            label_weights[:num_pos] = pos_weight # 设置正样本的权重
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode(
                    pos_bboxes, pos_gt_bboxes) # 编码正样本框和真实边界框
            else:
                # When the regression loss (e.g. `IouLoss`, `GIouLoss`)
                # is applied directly on the decoded bounding boxes, both
                # the predicted boxes and regression targets should be with
                # absolute coordinate format.
                pos_bbox_targets = pos_gt_bboxes # 如果 reg_decoded_bbox 为 True，则直接使用真实边界框作为目标。
            bbox_targets[:num_pos, :] = pos_bbox_targets
            bbox_weights[:num_pos, :] = 1 # 将正样本的边界框权重设为 1，这意味着正样本会参与回归损失的计算
        if num_neg > 0:
            label_weights[-num_neg:] = 1.0 # 负样本会参与分类损失的计算，但不参与回归损失的计算

        return labels, label_weights, bbox_targets, bbox_weights # 返回标签、权重、目标框和权重

    def get_targets(self,
                    sampling_results, #每个元素是一张图像的 SamplingResults 对象，记录正负样本的分配情况
                    gt_bboxes, # 每张图像的gt_bboxes
                    gt_labels, # 每张图像的gt_labels   
                    rcnn_train_cfg, # 训练配置
                    concat=True):
        ...
        if concat:
            labels = torch.cat(labels, 0) #所有提议框的分类标签
            label_weights = torch.cat(label_weights, 0) #所有提议框的分类权重
            bbox_targets = torch.cat(bbox_targets, 0) #所有提议框的回归目标
            bbox_weights = torch.cat(bbox_weights, 0) #所有提议框的回归权重
