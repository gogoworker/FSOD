...
"""《MHAF-YOLO: Multi-Branch Heterogeneous Auxiliary Fusion YOLO for accurate object detection》Arxiv2025
由于路径聚合 FPN（PAFPN）有效的多尺度特征融合能力，它已经成为基于 YOLO 的检测器中广泛采用的组件。
然而，PAFPN 难以将高级语义线索与低级空间细节相结合，限制了其在实际应用中的性能，尤其是在尺度变化较大的情况下。
在本文中，我们提出了 MHAF-YOLO，一种新颖的检测框架，具有称为多分支辅助 FPN（MAFPN）的多功能颈部设计，
它由两个关键模块组成：浅层辅助融合（SAF）和高级辅助融合（AAF）。SAF 通过融合浅层特征来连接主干和颈部，有效地以高保真度传输关键的低级空间信息。
同时，AAF 在更深的颈部层集成多尺度特征信息，为输出层提供更丰富的梯度信息，进一步增强模型学习能力。
为了补充 MAFPN，我们引入了全局异构灵活核选择 (GHFKS) 机制和重参数化异构多尺度 (RepHMS， Reparameterized Heterogeneous Multi-Scale) 模块来增强特征融合。
RepHMS 全局集成到网络中，利用 GHFKS 为各个特征层选择更大的卷积核，扩展垂直感受野并捕获跨空间层次的上下文信息。
在局部，它通过在同一层内处理大核和小核来优化卷积，扩大横向感受野并保留检测较小目标的关键细节
。这项工作的源代码可在以下位置获得：https://github.com/yang0201/MHAF-YOLO。
"""
...
class RepHMS(nn.Module):
    def __init__(self, in_channels, out_channels, width=3, depth=1, depth_expansion=2, kersize=5, shortcut=True,
        ...
        for _ in range(width - 1):
            DepthBlock = nn.ModuleList([self.width = width
...
if __name__ == '__main__':
    block = RepHMS(in_channels=64, out_channels=64, width=3, depth=1, depth_expansion=2, kersize=5, shortcut=True)
